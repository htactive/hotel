...
namespace Hotel.WebBase.Authentication
{
    ...
    public class HTAuthorizationHandler : IAuthorizationHandler
    {
        ...
        public HTAuthorizationHandler(InstanceRepository _BaseDBRepository)
        {
            this.Repository = _BaseDBRepository;
        }
        public Task HandleAsync(AuthorizationHandlerContext context)
        {
            ...
            Repository.UserLoginTokenRepository.Save(loginSession);
            Repository.Commit();
            ...
            {
#if DEBUG
                var userRole = Repository.UserRoleRepository.GetAll().Where(x => x.UserId.HasValue && x.UserId.Value == loginSession.UserId).Select(x => x.Role).OrderByDescending(x => x.RoleType).FirstOrDefault();
        ...
        {
            return Repository.ClaimRepository.GetAll().Any(x => x.ClaimName == claim);
        }
        private void CreateNewClaim(string claim)
        {
            ...
            Repository.ClaimRepository.Save(entity);
            Repository.Commit();
        }

        private bool CheckClaims(string requireClaim, int userId)
        {
            return Repository.UserRoleRepository.GetAll()
